...
def hero_jaccard_distance(heros_superpowers, hero_name = 'A-Bomb', features = ['Agility', 'Accelerated Healing', 'Lantern Power Ring']):
    result = []
    ...
    for index, row in heros_indexed_by_features.iterrows():
        result.append(distance.jaccard(main_hero, row))
    
    heros_indexed_by_features.insert(0, "Score", result, True)
    return heros_indexed_by_features


hero_jaccard_distance(heros_superpowers, sys.argv[1], sys.argv[2].split(',')).sort_values(by=['Score']).head(15).to_json('result.json')
