...
def hero_dice_distance(heros_superpowers, hero_name = 'A-Bomb', features = ['Agility', 'Accelerated Healing', 'Lantern Power Ring']):
    result = []
...
hero_dice_distance(heros_superpowers, sys.argv[1], sys.argv[2].split(',')).sort_values(by=['Score']).head(15).to_json('result.json')
